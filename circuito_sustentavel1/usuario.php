...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['excluir_conta'])) {
    ...
    if ($res && $res->num_rows > 0) {
        ...
        if (password_verify($senha, $row['senha'])) {
            ...
            $conexao->query("DELETE FROM Assinatura WHERE id_cliente = '$id_cliente'");
            $conexao->query("DELETE FROM Pedido WHERE id_cliente = '$id_cliente'");
            $conexao->query("DELETE FROM Carrinho WHERE id_cliente = '$id_cliente'");
